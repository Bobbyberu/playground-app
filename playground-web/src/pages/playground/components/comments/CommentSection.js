...
class CommentSection extends React.Component {
  ...
  render() {
    ...
    if (comments === 0) {
      commentsDisplay = (
        <div>
          <h2>Chargement mon petit pote</h2>
        </div>
      );
    } else {
    ...
    return (
      <div id="comment-section">
        <div className={classes.topRow + " row col-12 justify-content-between"}>
          <div className={classes.sectionTitle} role="img" aria-label="note">
            <h5>
              {" "}
          ...
          {AuthService.isUser() && (
            <div>
              ...
              >
                <AddComment className={classes.addIcon} />
                Rédiger un avis
              </Fab>
            </div>
          )}
        </div>
        <div className="row col-12">{commentsDisplay}</div>
      </div>
    );
  }
}
