...
class AddCommentModal extends React.Component {
    ...
    render() {
        ...
        return (
            <Dialog open={open} onEnter={this.handleEnter.bind(this)} onClose={this.handleClose.bind(this)} maxWidth={"md"} fullWidth>
    ...
    renderForm(classes) {
        ...
        return (
            <React.Fragment>
                <DialogTitle><span role="img">📝</span> Rédiger un avis</DialogTitle>
                <ValidatorForm ref="form"
                    onSubmit={this.handleSubmit}
                    onError={errors => console.log(errors)}>
                    <DialogContent>
                        <div>
                            <DialogContentText>Veuillez renseigner une note de 0 à 5.</DialogContentText>
                            <FormControl variant="outlined" className={classes.selectMark}>
                                <Select
                                    value={this.state.mark}
                                    onChange={this.handleInputChange}
                                    input={
                                        <OutlinedInput labelWidth={0} name="mark" id="outlined-age-simple" />
                                    }
                                    displayEmpty
                                >
                                    {marks.map(index => (
                                        <MenuItem key={index} value={index}>{index.toString()}</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                        </div>
